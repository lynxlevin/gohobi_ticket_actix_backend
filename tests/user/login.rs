...
async fn block_too_many_attempts_on_incorrect_password() -> Result<(), DbErr> {
    ...
    let user = factory::user().password(argon2_password).insert(&db.db).await?;
...
async fn not_found_on_incorrect_email() -> Result<(), DbErr> {
    ...
    let req = test::TestRequest::post()
        .uri("/api/users/login")
        .set_json(LoginRequest { email: "incorrect-email@test.com".to_string(), password: password.to_string() })
        .to_request();
...
#[actix_web::test]
#[ignore]
async fn django_password_should_be_updated_to_argon2_password_on_successful_login() -> Result<(), DbErr> {
    ...
    let user = factory::user().password(django_password).insert(&db.db).await?;
    ...
    let user_in_db = users_user::Entity::find_by_id(user.id).one(&db.db).await?.unwrap();
